...
public sealed class GroqChatProvider : IChatProvider
{
    ...
    private readonly ILogger<GroqChatProvider> _log;
    private readonly CloudCircuitBreaker _circuit;

    public GroqChatProvider(ServerSettings settings, IHttpClientFactory httpFactory, ILogger<GroqChatProvider> log)
    {
        _settings = settings;
        _httpFactory = httpFactory;
        _log = log;
        _circuit = new CloudCircuitBreaker("groq", log);
    }

    public ModelSource Source => ModelSource.Groq;

    public bool IsReady(CatalogEntry entry) => _settings.IsGroqConfigured;

    public string? UnavailableReason(CatalogEntry entry) =>
        IsReady(entry) ? null : "Groq API key is not configured. Open Server Settings → Cloud keys (global admin only).";

    public Task LoadAsync(CatalogEntry entry, string? localFilePath, CancellationToken ct)
    {
        if (!_settings.IsGroqConfigured)
    ...
    private async IAsyncEnumerable<string> GenerateCoreAsync(
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        var key = _settings.GetGroqApiKey()
            ?? throw new InvalidOperationException("Groq API key is not configured.");
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException($"Groq returned {(int)resp.StatusCode}: {Truncate(err, 400)}");
        }
        ...
        while (!reader.EndOfStream)
        {
            ...
            {
                _log.LogDebug(ex, "Groq: ignoring malformed SSE payload ({Snippet})", Truncate(payload, 120));
                continue;
            }
